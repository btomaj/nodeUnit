 ...
 */
(function () {
    var _require = typeof require !== "undefined" ? require : function () {
            return {argv: {slice: function() {return [];}}};
        },
        fs = _require("fs"),
        vm = _require("vm"),
        path = _require("path"),
     ...
     * Tests a test suite.
     *
     * A test suite is an object composed of an arbitrary number of methods,
     * each of which is a test case (http://en.wikipedia.org/wiki/XUnit).
     *
     ...
     * methods are not test cases, therefore exceptions and/or assertions are
     * not captured for these methods.
     *
     * All other methods are treated as test cases. Properties are permitted but
     * ignored.
     *
     * All methods of the test suite can refer to the test case object using
     * "this".
     *
     * TODO
    ...
    async function evaluateTestSuite(testSuite) {
        ...
        console.log(testsRun + " test(s) run; " + testsFailed +
                " test(s) failed.");
    }
     ...
     * @method loadFile
     *
     * @param filePath {string} Relative path from the test file to the file to
     *          load
     *
     * @static
     * @private
    ...
    function loadFile(filePath) {
        ...
        vm.runInThisContext(fs.readFileSync(path.join(caller, filePath),
                "utf-8"));
    }
